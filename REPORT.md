...
> **観測モデルの誤差 約1秒あたり検知遅延 約1.7分の代償**（exit-timing 誤差 8秒→74秒 で
> 代償 0.2分→113分に跳ねる；単一PIRは長期臥床の多くを取りこぼす）。**モダリティ群別FLは
> 素朴FedAvg比で代償を 78%圧縮**（サポート欠損比0.3；欠損比0.7では99.9%）。**PIR×1＋圧力は
> PIR×N（数の増設）と比べ、被覆率0.8回復に対し同じ+1センサで recall 1.0 を達成し〈安い〉**
> （種類＝在床直接観測による回復）。
