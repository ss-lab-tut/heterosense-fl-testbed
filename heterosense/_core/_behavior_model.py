    ...
    def generate(self, n_steps: int) -> list[LatentState]:
        ...
        for t in range(n_steps):
            ...
            if current_state != SemanticState.ABNORMAL and next_state == SemanticState.ABNORMAL:
                ...
                if mode < 0.15:
                    abnormal_target = int(self.rng.integers(2, 4))    # near fall
                elif mode < 0.85:
                    abnormal_target = int(self.rng.integers(5, 11))   # fall + early post-fall
                else:
                    abnormal_target = int(self.rng.integers(12, 25))  # prolonged immobility
                abnormal_motion_pattern = str(self.rng.choice([
                    "STILL",
                    "ROLL",
                    "LIMB_MOVEMENT",
                    "SIT_UP_ATTEMPT",
            ...
            if current_state == SemanticState.ABNORMAL:
